...
# Carregar os dados
caminho_dados = "../Caracteres-faussett/"
dados = pd.read_csv(caminho_dados + "caracteres-limpo.csv", header=None)
dados_teste= pd.read_csv(caminho_dados + "caracteres-ruido.csv", header=None)   # Assume que não há cabeçalho
...
X_treinamento, X_validacao, Y_treinamento, Y_validacao = train_test_split(
    X, Y, test_size=0.2, random_state=42  # 80% para treinamento, 20% para validação
)

# Parâmetros da rede
n_entradas = X_treinamento.shape[1]
n_camadas_escondidas = [20]
n_saida = 7
...
resultado_treinamento = mlp_backpropagation(modelo, X_treinamento, Y_treinamento, learning_rate=0.1, threshold=0.000000001, num_epocas=10000, X_validacao=X_validacao, Y_validacao=Y_validacao)
...
saidas_teste = mlp_predict(modelo, X)
