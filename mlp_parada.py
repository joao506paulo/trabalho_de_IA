...
# Backpropagation
def mlp_backpropagation(modelo, X_treinamento, Y_treinamento, learning_rate, num_epocas, X_validacao=None, Y_validacao=None, paciencia=10):
    ...
    for epoca in range(num_epocas):
        ...
        mse_treino = erro_total / num_amostras
        erros_treino.append(mse_treino)

        # Validação
        if X_validacao is not None and Y_validacao is not None:
            mse_validacao = calcular_erro(modelo, X_validacao, Y_validacao)
            erros_validacao.append(mse_validacao)

            print(f"Época {epoca + 1}, Erro Quadrático Médio (Treino): {mse_treino}, Erro Quadrático Médio (Validação): {mse_validacao}")

            # Parada antecipada
            if mse_validacao < melhor_erro_validacao:
                melhor_erro_validacao = mse_validacao
                contador_paciencia = 0
                melhor_modelo = [p.copy() for p in modelo['pesos']]  # Salva os melhores pesos
            else:
                contador_paciencia += 1
                if contador_paciencia >= paciencia:
                    print(f"Parada antecipada ativada na época {epoca + 1}.")
                    modelo['pesos'] = melhor_modelo  # Restaura os melhores pesos
                    return {'modelo': modelo, 'epocas_treinadas': epoca + 1, 'erros_treino': erros_treino, 'erros_validacao': erros_validacao}
        else:
            print(f"Época {epoca + 1}, Erro Quadrático Médio (Treino): {mse_treino}")

    return {'modelo': modelo, 'epocas_treinadas': num_epocas, 'erros_treino': erros_treino, 'erros_validacao': erros_validacao}
...
resultado_treinamento = mlp_backpropagation(modelo, X_treinamento, Y_treinamento, learning_rate=0.005, num_epocas=1000, X_validacao=X_validacao, Y_validacao=Y_validacao, paciencia=10)
...
with open("erros_por_epoca.txt", "w") as f:
    for erro in resultado_treinamento['erros_treino']:
        f.write(str(erro) + "\n")
    if X_validacao is not None and Y_validacao is not None:
        f.write("\nErros de Validação:\n")
        for erro in resultado_treinamento['erros_validacao']:
            f.write(str(erro) + "\n")
